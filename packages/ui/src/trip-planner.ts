...
export class TripRoutePlannerElement extends HTMLElement {
  ...
  private handleClick = (event: Event): void => {
    ...
    switch (target.dataset.action) {
      case "add-stop":
        this.draft.stops = insertStop(this.draft.stops, this.draft.stops.length);
        break;
      case "insert-stop":
        this.draft.stops = insertStop(this.draft.stops, index + 1);
        break;
      case "duplicate-stop": {
        const stop = this.draft.stops[index];
        if (stop) this.draft.stops = insertStop(this.draft.stops, index + 1, duplicateStop(stop));
        break;
      }
      case "remove-stop":
  ...
  private renderStop(stop: TripStop, index: number): string {
    ...
    const insertionAllowed = canInsertStopAt(this.draft.stops, index + 1);
