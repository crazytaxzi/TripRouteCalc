...
function dailyDutyText(values: readonly DailyDutyTotal[]): string {
  return values
    .map((entry): string => `${entry.date},${String(entry.onDutyMinutes)}`)
    .join('\n');
}

function sleeperPeriodText(values: readonly SleeperPeriod[]): string {
  return values
    .map((entry): string => `${entry.startAt},${entry.endAt}`)
    .join('\n');
}

function recapText(state: TripSetupState): string {
  return state.hos.cycleRecaps
    .map(
      (entry): string =>
...
function parseDailyDutyTotals(value: string): readonly DailyDutyTotal[] {
  return value
    .split(/\r?\n/u)
    .map((line): string => line.trim())
    .filter((line): boolean => line !== '')
    .map((line): DailyDutyTotal => {
      const [date = '', minutes = ''] = line.split(',');
      return { date: date.trim(), onDutyMinutes: Number(minutes.trim()) };
    });
}

function parseSleeperPeriods(value: string): readonly SleeperPeriod[] {
  return value
    .split(/\r?\n/u)
    .map((line): string => line.trim())
    .filter((line): boolean => line !== '')
    .map((line): SleeperPeriod => {
      const [startAt = '', endAt = ''] = line.split(',');
      return { startAt: startAt.trim(), endAt: endAt.trim() };
    });
}

function parseRecaps(value: string): TripSetupState['hos']['cycleRecaps'] {
  return value
    .split(/\r?\n/u)
    .map((line): string => line.trim())
    .filter((line): boolean => line !== '')
...
function renderHos(state: TripSetupState): string {
  const hos = state.hos;
  return `<details class="subpanel" open>
    <summary>Hours-of-service departure facts</summary>
    <p class="hint">Enter recorded facts only. TripRouteCalc does not infer legal history from the three remaining-clock values.</p>
    <div class="grid two">
      <label class="field"><span>Cycle</span><select name="hos.cycleType"><option value="">Choose cycle</option><option value="60_in_7"${hos.cycleType === '60_in_7' ? ' selected' : ''}>60 hours in 7 days</option><option value="70_in_8"${hos.cycleType === '70_in_8' ? ' selected' : ''}>70 hours in 8 days</option></select></label>
      <label class="field"><span>Fact source</span><select name="hos.provenance"><option value="">Choose source</option><option value="user_entered"${hos.provenance === 'user_entered' ? ' selected' : ''}>User entered</option><option value="imported_eld"${hos.provenance === 'imported_eld' ? ' selected' : ''}>Imported ELD</option><option value="carrier_record"${hos.provenance === 'carrier_record' ? ' selected' : ''}>Carrier record</option></select></label>
      ${minutesInput('hos.drivenSinceQualifyingInterruptionMinutes', 'Driven since qualifying interruption', hos.drivenSinceQualifyingInterruptionMinutes)}
      ${minutesInput('hos.onDutyCurrentShiftMinutes', 'On duty in current shift', hos.onDutyCurrentShiftMinutes)}
      ${minutesInput('hos.offDutyBeforeDepartureMinutes', 'Off duty immediately before departure', hos.offDutyBeforeDepartureMinutes)}
      ${minutesInput('hos.carrierMaximumDrivingMinutes', 'Carrier driving limit', hos.carrierMaximumDrivingMinutes)}
      ${minutesInput('hos.carrierMaximumDutyMinutes', 'Carrier duty limit', hos.carrierMaximumDutyMinutes)}
    </div>
    <div class="grid two">
      <label class="check"><input name="hos.qualifyingTenHourBreakCompleted" type="checkbox"${hos.qualifyingTenHourBreakCompleted ? ' checked' : ''}> Qualifying 10-hour break completed</label>
      <label class="check"><input name="hos.sleeperBerthEligible" type="checkbox"${hos.sleeperBerthEligible ? ' checked' : ''}> Sleeper-berth eligible</label>
      <label class="check"><input name="hos.splitSleeperEnabled" type="checkbox"${hos.splitSleeperEnabled ? ' checked' : ''}> Use split-sleeper planning</label>
      <label class="check"><input name="hos.plannedThirtyFourHourRestart" type="checkbox"${hos.plannedThirtyFourHourRestart ? ' checked' : ''}> Plan a 34-hour restart</label>
      <label class="check"><input name="hos.restPreference.enabled" type="checkbox"${hos.restPreference.enabled ? ' checked' : ''}> Prefer a nightly rest window</label>
    </div>
    <div class="grid two">
      <label class="field"><span>Preferred rest start</span><input name="hos.restPreference.startLocalTime" type="time" value="${escapeHtml(hos.restPreference.startLocalTime)}"></label>
      <label class="field"><span>Preferred rest end</span><input name="hos.restPreference.endLocalTime" type="time" value="${escapeHtml(hos.restPreference.endLocalTime)}"></label>
    </div>
    <div class="grid three">
      <label class="field"><span>Prior daily duty totals</span><textarea name="hos.priorDutyTotals" rows="7" placeholder="2026-07-22,540">${escapeHtml(dailyDutyText(hos.priorDutyTotals))}</textarea><small>One line per day: date,on-duty minutes. Enter six days for 60/7 or seven days for 70/8.</small></label>
      <label class="field"><span>Cycle recaps</span><textarea name="hos.cycleRecaps" rows="7" placeholder="2026-07-24T00:00,480">${escapeHtml(recapText(state))}</textarea><small>One line per return: available time,minutes returning.</small></label>
      <label class="field"><span>Existing sleeper periods</span><textarea name="hos.existingSleeperPeriods" rows="7" placeholder="2026-07-22T20:00,2026-07-23T03:00">${escapeHtml(sleeperPeriodText(hos.existingSleeperPeriods))}</textarea><small>One line per period: start,end.</small></label>
    </div>
  </details>`;
}

function renderStop(
  stop: TripSetupState['stops'][number],
  count: number,
): string {
  const sequence = String(stop.sequence + 1);
  const types = STOP_TYPES.map(
    (type): string =>
      `<option value="${type}"${type === stop.type ? ' selected' : ''}>${type.replaceAll('_', ' ')}</option>`,
  ).join('');
...
export class TripSetupApp {
  ...
  public constructor(root: HTMLElement) {
    this.#root = root;
    this.#state =
      restoreDraft(localStorage.getItem(STORAGE_KEY) ?? '') ??
      createInitialTripSetupState();
    this.#api = new TripSetupApiClient(
      root.dataset.apiBaseUrl ?? '/api',
      (): string => sessionStorage.getItem('trip-route-calc-token') ?? '',
    );
    this.#recalculate = new RecalculationController(
      async (): Promise<void> => {
  ...
  #setState(state: TripSetupState, focusSelector?: string): void {
    ...
    if (focusSelector !== undefined) {
      this.#root.querySelector<HTMLElement>(focusSelector)?.focus();
    }
  ...
  #render(): void {
    ...
    const status = this.#state.calculationPending
      ? 'Saving and calculating...'
      : (this.#state.lastError ??
        (this.#state.tripId === undefined
          ? 'Draft saved on this device'
          : `Server trip saved at revision ${String(this.#state.revisionNumber)}`));
  ...
  #bind(): void {
    ...
    form?.addEventListener('change', (event): void => {
      this.#handleFormInput(event);
    });
    this.#root
      .querySelector('[data-global-action="add-stop"]')
      ?.addEventListener('click', (): void => {
        this.#setState(insertStop(this.#state, this.#state.stops.length - 1));
      });
    this.#root
      .querySelector('[data-global-action="save"]')
      ?.addEventListener('click', (): void => {
        void this.#save();
      });
    this.#root
      .querySelector('[data-global-action="calculate"]')
      ?.addEventListener('click', (): void => {
        void this.#calculate();
      });
    this.#root.querySelectorAll<HTMLElement>('.stop-card').forEach((card): void => {
      ...
      card
        ...
        ?.addEventListener('click', (): void => {
          this.#setState(moveStop(this.#state, localId, -1));
        });
      card
      ...
      card
        ...
        ?.addEventListener('click', (): void => {
          this.#setState(duplicateStop(this.#state, localId));
        });
      card
        .querySelector('[data-action="insert-after"]')
        ?.addEventListener('click', (): void => {
          this.#setState(insertStop(this.#state, stopIndex(card) + 1));
        });
      card
        .querySelector('[data-action="remove"]')
        ?.addEventListener('click', (): void => {
          this.#setState(removeStop(this.#state, localId));
        });
      card.addEventListener('dragstart', (): void => {
        this.#draggedStopId = localId;
      });
      card.addEventListener('dragover', (event): void => {
        event.preventDefault();
      });
      card.addEventListener('drop', (event): void => {
        event.preventDefault();
        this.#dropStop(localId);
      });
    });
  }
  ...
  #dropStop(targetId: string): void {
    ...
    const targetIndex = this.#state.stops.findIndex(
      (stop): boolean => stop.localId === targetId,
    );
    let next = this.#state;
    const direction: -1 | 1 =
      next.stops.findIndex((stop): boolean => stop.localId === dragged) < targetIndex
        ? 1
        : -1;
    while (
      next.stops.findIndex((stop): boolean => stop.localId === dragged) !== targetIndex
  ...
  #handleFormInput(event: Event): void {
    ...
    if (card !== null) {
      ...
      if (localId !== undefined && field !== undefined) {
        this.#updateStopField(localId, field, target);
      }
      return;
    }
    const name = target.name;
    if (name.startsWith('hos.')) {
      this.#updateHosField(name, target);
    } else if (
      name === 'autoRecalculate' &&
      target instanceof HTMLInputElement
    ) {
      this.#setState({ ...this.#state, autoRecalculate: target.checked });
    } else if (
      name === 'departureAt' ||
      name === 'departureTimeZone' ||
      name === 'currentDutyStatusBeganAt'
    ) {
      this.#setState({ ...this.#state, [name]: target.value, dirty: true });
    } else if (name === 'currentDutyStatus') {
      this.#setState({
        ...this.#state,
        currentDutyStatus: target.value as TripSetupState['currentDutyStatus'],
        dirty: true,
      });
    } else if (name === 'driver') {
      this.#setState({
        ...this.#state,
        driver: { selectedId: '', displayName: target.value },
        dirty: true,
      });
    } else if (name === 'tractor' || name === 'trailer' || name === 'load') {
      this.#setState({
        ...this.#state,
        [name]: { selectedId: target.value.trim(), displayName: target.value },
        dirty: true,
      });
    } else if (name in this.#state.clocks) {
      this.#setState({
        ...this.#state,
        clocks: { ...this.#state.clocks, [name]: Number(target.value) },
        dirty: true,
      });
    }
  }
  ...
  #updateHosField(
    ...
    const field = name.slice('hos.'.length);
    const hos = this.#state.hos;
    if (field === 'priorDutyTotals') {
      this.#setState({
        ...this.#state,
        hos: { ...hos, priorDutyTotals: parseDailyDutyTotals(target.value) },
        dirty: true,
      });
      return;
    }
    if (field === 'cycleRecaps') {
      this.#setState({
        ...this.#state,
        hos: { ...hos, cycleRecaps: parseRecaps(target.value) },
        dirty: true,
      });
      return;
    }
    if (field === 'existingSleeperPeriods') {
      this.#setState({
        ...this.#state,
        hos: {
          ...hos,
          existingSleeperPeriods: parseSleeperPeriods(target.value),
        },
        dirty: true,
      });
      return;
    }
    if (field.startsWith('restPreference.')) {
      const key = field.slice('restPreference.'.length);
      const value =
      ...
      this.#setState({
        ...
        hos: {
          ...hos,
          restPreference: { ...hos.restPreference, [key]: value },
        },
        dirty: true,
      });
      return;
  ...
  #updateStopField(
    ...
    const stop = this.#state.stops.find(
      (candidate): boolean => candidate.localId === localId,
    );
    ...
    if (field.startsWith('appointment.')) {
        ...
        target instanceof HTMLInputElement && target.type === 'checkbox'
          ...
          : key === 'lateToleranceMinutes'
            ...
            : target.value === ''
              ...
              : target.value;
      this.#setState(
        updateStop(this.#state, localId, {
          appointment: { ...stop.appointment, [key]: value },
        }),
      );
    } else if (field.startsWith('service.')) {
      const key = field.slice('service.'.length);
      const value = key.endsWith('Minutes')
        ? target.value === ''
          ? undefined
          : Number(target.value)
        : target.value;
      this.#setState(
        updateStop(this.#state, localId, {
          service: { ...stop.service, [key]: value },
        }),
      );
    } else {
      const value =
  ...
  async #calculate(): Promise<void> {
    ...
    try {
      ...
      this.#setState({
        ...
        lastCalculationAt: new Date().toLocaleString(),
        lastError: undefined,
      });
    } catch (error) {
      this.#setFailure(error);
  ...
  #setFailure(error: unknown): void {
    const message =
      error instanceof TripSetupValidationError
        ? error.issues.map((issue): string => issue.message).join(' ')
        : error instanceof Error
          ? error.message
          : 'The request failed. Your draft is preserved.';
    this.#setState({
      ...this.#state,
      calculationPending: false,
      lastError: message,
