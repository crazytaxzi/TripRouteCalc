...
export const STOP_TYPES = ['start_location','tractor_pickup','trailer_pickup','shipper','intermediate_pickup','intermediate_delivery','final_consignee','fuel','scale','inspection','maintenance','food','driver_break','sleeper_rest','terminal','border_crossing','other'] as const;
...
export interface ProfileSelection { readonly selectedId: string; readonly displayName: string; }
...
export function createStop(type: StopType, sequence: number): TripStopDraft {
  ...
  return { localId: `local-stop-${String(nextLocalStopId++)}`, sequence, type, label: '', address: '', required: true, lockedPosition: type === 'start_location' || type === 'final_consignee', appointment: { mode: 'none', timeZone: 'America/Boise', lateToleranceMinutes: 0, earlyParkingAllowed: false, overnightParkingAllowed: false }, service: { mode: 'expected', expectedMinutes: defaultMinutes, dutyStatus: 'on_duty_not_driving' }, notes: '' };
}
...
function validateService(stop: TripStopDraft): readonly ValidationIssue[] { const issues: ValidationIssue[] = []; if (stop.service.mode === 'exact' && !(Number.isFinite(stop.service.exactMinutes) && (stop.service.exactMinutes ?? 0) >= 0)) issues.push({ path: `stops.${String(stop.sequence)}.service.exactMinutes`, message: 'Enter an exact service duration.', severity: 'error' }); if (stop.service.mode === 'expected' && !(Number.isFinite(stop.service.expectedMinutes) && (stop.service.expectedMinutes ?? 0) >= 0)) issues.push({ path: `stops.${String(stop.sequence)}.service.expectedMinutes`, message: 'Enter an expected service duration.', severity: 'error' }); if (stop.service.mode === 'range') { const minimum = stop.service.minimumMinutes ?? -1; const maximum = stop.service.maximumMinutes ?? -1; if (minimum < 0 || maximum < minimum) issues.push({ path: `stops.${String(stop.sequence)}.service`, message: 'Service range must have a nonnegative minimum and a maximum at least as large.', severity: 'error' }); } return issues; }
...
export function validateTripSetup(state: TripSetupState): readonly ValidationIssue[] {
  ...
  for (const [key, value] of Object.entries(state.clocks)) if (!Number.isFinite(value) || value < 0) issues.push({ path: `clocks.${key}`, message: 'Clock values must be nonnegative.', severity: 'error' });
  ...
  state.stops.forEach((stop): void => { const sequence = String(stop.sequence); if (stop.address.trim() === '') issues.push({ path: `stops.${sequence}.address`, message: `Stop ${String(stop.sequence + 1)} needs a location or address.`, severity: 'error' }); if (stop.appointment.mode === 'fixed' && stop.appointment.fixedAt === undefined) issues.push({ path: `stops.${sequence}.appointment.fixedAt`, message: 'Enter the fixed appointment time.', severity: 'error' }); if (stop.appointment.mode === 'window' && (stop.appointment.earliestAt === undefined || stop.appointment.latestAt === undefined)) issues.push({ path: `stops.${sequence}.appointment`, message: 'Enter both ends of the appointment window.', severity: 'error' }); issues.push(...validateService(stop)); });
  return issues;
}
