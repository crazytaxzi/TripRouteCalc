# 06. HOS Cycle Limits, Daily Recaps, and 34-Hour Restart
...
- Source 05 core HOS engine passing
- Historical on-duty data model available

## Goal

Extend the pure HOS engine with 60-hour/7-day and 70-hour/8-day cycles, daily recap returns, and an explicitly planned qualifying 34-hour restart.

## Required work

1. Calculate rolling cycle consumption from timestamped on-duty events, not trusted totals alone.
2. Support both configured cycle types.
3. Reconcile entered cycle remaining against history and report discrepancies rather than silently replacing data.
4. Calculate recap hours at the correct home-terminal or configured regulatory boundary.
5. Expose upcoming recap returns as timestamped availability events.
6. Prevent driving and on-duty planning when cycle availability is exhausted.
7. Implement qualifying 34-consecutive-hour restart behavior.
8. Select the earliest legally sufficient continuation:
   - use recap hours when they return sooner and the plan permits
   - use 10-hour rest when only daily clocks need restoration
   - use a 34-hour restart only when explicitly planned or operationally selected
9. Return structured reasons for cycle-limited delays.
10. Add boundary, DST, time-zone, and historical-window tests.

## Required scenarios

- More shift time than cycle time
- Cycle reaches zero during a route
- Recap hours return at the configured boundary
- Driver cannot depart with zero cycle time
- Qualifying 34-hour restart resets the applicable cycle
- Non-qualifying off-duty period does not reset cycle
- Recap and restart options produce transparent different timelines

## Hard boundaries

- Do not “award” recap hours at an arbitrary calendar midnight.
- Do not assume a restart.
- Do not mutate original duty history to make entered totals agree.
- Do not merge cycle calculations into ETA code.

## Exit gate

The engine can explain exactly when cycle hours are consumed, returned, or reset, and the planner can query the next legal availability.
