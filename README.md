...
The canonical private repository is established on `main`. Stages 01 through 15 are complete, verified, merged, and ledger-closed. Stage 16 is the exact next source.
...
Begin `docs/specification/16_CONFIDENCE_EXPLANATIONS_DATA_QUALITY.md` under the Prime Directive, preserving the accepted HOS, stop-order, equipment, route, compliance, operational-event, time-zone, ETA, persistence, immutable-revision, and audit boundaries.
